...
## What It Does
Tracks tool call activity across the session -- writes to TASKS.md, SCORECARD.md, CONVERSATIONS.md, message sends, tool call counts, skill consultations, OB context loads, and review agent spawns. Never blocks any action.
...
2. Resets `toolCallsSinceMessage` to 0 on message sends.
3. Increments `toolCallsSinceMessage` for non-compliance exec/bash calls.
4. Sets `skillConsultedThisTurn` when SKILL files are read or catted.
5. Sets per-session context flags (`tasksReadThisSession`, `conversationsReadThisSession`, `obContextLoadedThisSession`) when those resources are accessed.
6. Sets `reviewAgentSpawned` when an agent is spawned during active PR review context.
