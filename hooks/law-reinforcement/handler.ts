...
1. **OB FIRST** -- Before asking the operator ANY factual question, run: \`~/.local/bin/mcp2cli open-brain search_all --params '{"query": "..."}'\`. If OB has the answer, USE IT. Only ask the operator if OB doesn't have it. Say "Checked OB, didn't find it" when you do ask.
...
8. ANNOUNCE EVERY STEP: Say "Starting Step X..." before, "Done with Step X" after. Update the operator every 2-5 min on long tasks. NEVER go silent.
9. If unsure whether to do something, ASK the operator first -- do not assume
...
13. You CANNOT fix your own infrastructure -- ask the operator to make config/infra changes
